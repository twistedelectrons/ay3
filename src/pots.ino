...
{
    potJob++;
    if (potJob > 5) potJob=1;
   
    switch (potJob) {
        case 1: analogTemp = analogRead(A4) >> 2;
                if (analogTemp > (potLast[0]+1) || analogTemp < (potLast[0]-1)) {   // RIO: fixed syntax, operator order could be compiler dep.
                    lfoSpeed = analogTemp;
        ...
        case 2: analogTemp = analogRead(A5) >> 2;
        ...
        case 3: analogTemp = analogRead(A7) >> 6;
        ...
        case 4: analogTemp = analogRead(A6) >> 2;
                if (analogTemp > (potLast[4]+1) || analogTemp < (potLast[4]-1)) {
                    ...
                    if (potLast[4] > -1) {
                        if (detune < 127)   ledNumber = numberOld;
        ...
        case 5: analogTemp = analogRead(A0) >> 2;
