...
void doButtons() 
{
    ...
    buttPhase++;
    if (buttPhase > 4) buttPhase = 0;

    switch (buttPhase) {

        //Phase 1
        case 0: PORTA &= ~_BV (1);  // digitalWrite (A1-25, LOW);
        ...
        //Phase 2
        case 1: PORTA |= _BV (1);   // digitalWrite (A1-25, HIGH);
        ...
        //Phase 3
        case 2: PORTA |= _BV (1);   // digitalWrite (A1-25, HIGH);
        ...
        //Phase 4
        case 3: PORTA |= _BV (1);   // digitalWrite (A1-25, HIGH);
                PORTA |= _BV (2);   // digitalWrite (A2-26, HIGH);
                PORTA |= _BV (3);   // digitalWrite (A3-27, HIGH);
...
void buttPressed(int pin, int state)
{
    ...
    if (pin == 1) {
        if (state == 1) {
            ...
            if (mode == 1)  { mode = 2; ledNumber = bank+1; }
            else            { mode = 1; ledNumber = preset+1; }
    ...
    if (state == 0) {
        ...
        if (seqSetup == 1) {
            if (pin == 5) { pressedRow = 2; encoderMoved(0); } //lfo pitch
            if (pin == 7) { pressedRow = 4; encoderMoved(0); } //env
        }
        ...
        if      (pin == 11) { pressedCol = 1; selectedChip = false; }
        else if (pin == 2)  { pressedCol = 2; selectedChip = false; }
        else if (pin == 6)  { pressedCol = 3; selectedChip = false; }
        else if (pin == 9)  { pressedCol = 4; selectedChip = true; }
        else if (pin == 12) { pressedCol = 5; selectedChip = true; }
        else if (pin == 3)  { pressedCol = 6; selectedChip = true; }

        if (pressedRow != 1) {
            voiceMode = 1; // RIO: reset voiceMode
            if      (pin == 11) bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
            else if (pin == 2)  bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
            else if (pin == 6)  bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
            else if (pin == 9)  bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
            else if (pin == 12) bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
            else if (pin == 3)  bitWrite(ledMatrix[pressedRow], pressedCol-1, !bitRead(ledMatrix[pressedRow], pressedCol-1));
        }
        ...
        if (pin == 11 || pin == 2 || pin == 6 || pin == 9 || pin == 12 || pin == 3) {
            ...
            if (pressedRow == 1 ||
                pressedRow == 3 ||
                pressedRow == 4) encoderMoved(0);
        }

    } else {

        //released
        if (pin == 8) voicePressed = false;
    }
}