void encoderMoved(int dir)
{
    if (seqSetup == 0) {
        ...
        if      (selectedStep > 15) selectedStep = 15;
        else if (selectedStep < 0)  selectedStep = 0;

        if (selectedStep <= 7)
            ledNumber = selectedStep+1; 
        else ledNumber = selectedStep-7;

        seqDisplayCounter = 0;

        switch (selectedStep) {

            case 0:     ledMatrixPic[1] = B010000;
        ...
        if (writeChannel) EEPROM.write(3802, selectedStep+1);
        ...
        if (voiceMode < 3) {
            ...
            if (pressedRow == 1 && pressedCol == 1) {
                ...
                if      (vol[pressedCol] > 15)  vol[pressedCol] = 15;
                else if (vol[pressedCol] < 0)   vol[pressedCol] = 0; 

                if (bitRead(ledMatrix[4], 0) == 1)
                    set_chA_amplitude(vol[pressedCol], true);
                else set_chA_amplitude(vol[pressedCol], false);
                ...
                else set_chB_amplitude(vol[pressedCol], false);
                ...
                else set_chC_amplitude(vol[pressedCol], false);
                ...
                else set_chD_amplitude(vol[pressedCol], false);
                ...
                else set_chE_amplitude(vol[pressedCol], false);
                ...
                else set_chF_amplitude(vol[pressedCol], false);
            ...
            if (pressedRow == 1 && pressedCol == 1) {
                ...
                if (presetTune[1] > 124)    presetTune[1] = 124;
                if (presetTune[1] < 76)     presetTune[1] = 76;

                if (!chord) tune[1] = presetTune[1];
            }

            if (pressedRow == 1 && pressedCol == 2) {
                presetTune[2] -= dir;

                if (presetTune[2] > 124)    presetTune[2] = 124;
                if (presetTune[2] < 76)     presetTune[2] = 76;

                if (!chord) tune[2] = presetTune[2];
            }
            
            if (pressedRow == 1 && pressedCol == 3) {
                presetTune[3] -= dir;
                if (presetTune[3] > 124)    presetTune[3] = 124;
                if (presetTune[3] < 76)     presetTune[3] = 76;

                if (!chord) tune[3] = presetTune[3];
            }

            if (pressedRow == 1 && pressedCol == 4) {
                presetTune[4] -= dir;

                if (presetTune[4] > 124)    presetTune[4] = 124;
                if (presetTune[4] < 76)     presetTune[4] = 76;

                if (!chord) tune[4] = presetTune[4];
            }
            
            if (pressedRow == 1 && pressedCol == 5) {
                presetTune[5] -= dir;

                if (presetTune[5] > 124)    presetTune[5] = 124;
                if (presetTune[5] < 76)     presetTune[5] = 76;
                
                if (!chord) tune[5] = presetTune[5];
            }

            if (pressedRow == 1 && pressedCol == 6) {
                presetTune[6] -= dir;
                if (presetTune[6] > 124)    presetTune[6] = 124;
                if (presetTune[6] < 76)     presetTune[6] = 76;

                if (!chord) tune[6] = presetTune[6];
            }
        }
        ...
        if (pressedRow == 2) {
            ...
            if (lfoShape > 8) lfoShape -= 8;
            if (lfoShape < 1) lfoShape += 8;
        ...
        if (pressedRow == 3) {
            if (!selectedChip) {
                ...
                if      (noiseFreq1 > 31)   noiseFreq1 = 31;
                else if (noiseFreq1 < 0)    noiseFreq1 = 0;
                ...
                if      (noiseFreq2 > 31)   noiseFreq2 = 31;
                else if (noiseFreq2 < 0)    noiseFreq2 = 0;
        ...
        if (pressedRow == 4) {
            if (!selectedChip) {
                ...
                if      (envNumber1 > 8) envNumber1 -= 8;
                else if (envNumber1 < 1) envNumber1 = 8;
                ...
                if      (envNumber2 > 8) envNumber2 -=8; 
                else if (envNumber2 < 1) envNumber2 = 8;

                ledNumber = envNumber2;

                if      (envNumber2 == 1) envShape2 = 0;
                else if (envNumber2 == 2) envShape2 = B0100;
                else if (envNumber2 == 3) envShape2 = B1000;
                else if (envNumber2 == 4) envShape2 = B1010;
                else if (envNumber2 == 5) envShape2 = B1011;
                else if (envNumber2 == 6) envShape2 = B1100;
                else if (envNumber2 == 7) envShape2 = B1101;
                else if (envNumber2 == 8) envShape2 = B1110;
            }
